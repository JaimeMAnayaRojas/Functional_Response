...
model{
  ...
  for ( j in 1:N ) {
    A[j] = exp(a + a_I * I[j] + a_i * i[j] + a_Intercept[ID[j]] + a_pD *pD[j] + a_Egg *Egg[j]);// + a_pD *pD[j] + a_cSize *cSize[j] + a_ISize *ISize[j] + a_Egg *Egg[j]
    H[j] = exp(h + h_I * I[j] + h_i * i[j] + h_Intercept[ID[j]] + h_pD *pD[j] + h_Egg *Egg[j]);//+ h_pD *pD[j] + h_cSize *cSize[j] + h_ISize *ISize[j] + h_Egg *Egg[j]
