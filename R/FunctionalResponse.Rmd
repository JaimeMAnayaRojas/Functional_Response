...
# fit <- readRDS("/Users/majadrakula/Desktop/Masterthesis/0_data/manual/model.RDS")
fit <- readRDS("../R/model.RDS")
...
p3 <- ggplot(df, aes(x = R, y = median, color = Treatment)) +
    geom_ribbon(aes(ymin = lower, ymax = upper, fill = Treatment), alpha = 0.25, show.legend = FALSE, size=0.01) +
...
AC = exp(post$a) # intercep represents the control
AI = exp(post$a + post$a_I) # add control + deviation from the control (intercept) by I
AnI = exp(post$a + post$a_i)

HC = exp(post$h)
HI = exp(post$h + post$h_I)
HnI = exp(post$h + post$h_i)

p_df <- cbind(AC, AI, AnI, HC, HI, HnI)

mediansEst <- apply(p_df, 2, median)

CIEst <- t(apply(p_df, 2, HPDI, prob =0.95))

dfEst <- data.frame(
    median = mediansEst,
    lower = CIEst[,1],
    upper = CIEst[,2],
    Treatment = c("C", "I", "nI", "C", "I", "nI" ),
    Parameter = c("a", "a", "a", "h", "h", "h")

)

dfEst <- dfEst %>%
  mutate(Treatment = factor(Treatment, levels=c("C", "nI", "I"))) 

# split data into h and a
dfa <- dfEst%>% 
        filter(Parameter != "h")
dfh <- dfEst%>% 
        filter(Parameter != "a")
...
aplot <- ggplot(dfa, aes(x=Treatment, y= median, col=Treatment, group=Treatment))+
         geom_point(size = 4, position = pd) + 
         geom_errorbar(aes(ymin = lower, ymax = upper) ,  position= pd, width = 0.2, size = 1, alpha= 0.8)+
         theme_classic() +
         labs(y="Attack Coefficient Estimates (attacks/hour)" ) +
        scale_color_manual(values=c("C"= "#339900", "I"= "#9966FF","nI"= "#FF9900"))+
        scale_fill_manual(values=c("C"= "#339900", "I"= "#9966FF","nI"= "#FF9900"))+
        theme(text = element_text(size = 12), axis.title = element_text(size = 12), axis.text=element_text(color="black"))+
        theme(legend.position ="none")+
...
hplot<- ggplot(dfh, aes(x=Treatment, y= median, col=Treatment, group=Treatment))+
         geom_point(size = 4, position = pd) + 
         geom_errorbar(aes(ymin = lower, ymax = upper) ,  position= pd, width = 0.2, size = 1, alpha= 0.8)+
         theme_classic() +
         labs(y="Handling time Estimates" ) +
        scale_color_manual(values=c("C"= "#339900","nI"= "#FF9900", "I"= "#9966FF"))+
        scale_fill_manual(values=c("C"= "#339900", "nI"= "#FF9900", "I"= "#9966FF"))+
        theme(text = element_text(size = 12), axis.title = element_text(size = 12), axis.text=element_text(color="black"))+
        theme(legend.position ="none")+
        theme(axis.title.x=element_blank())

PlotEstimates<-plot_grid(aplot, hplot, labels = "AUTO")

PlotEstimates
dev.print(pdf,"/Users/majadrakula/Desktop/Masterthesis/3_output/Figures_Tables/Figures/Estimates.pdf",width=6, height=4) 


```

### Effect of exposure on Functional Response 
```{r}
# clean up the workspace
rm(list = ls())
...
data2<-data %>% 
  ...
  filter(Treatment != "infected LV", Treatment != "double infection", Treatment != "triple infection") %>% 
  rename(R = "prey", Infection = "Treatment", Eaten = "prey consumed") %>% 
  mutate(E = ifelse(Infection ==  "control", 0,1)) 
...
saveRDS(fitE, "/Users/majadrakula/Desktop/Masterthesis/1_code/modelE.RDS")

#fitE <- readRDS("/Users/majadrakula/Desktop/Masterthesis/1_code/modelE.RDS")
...
# is the effect of infection on attack rate positive?
100-LOS(postE$a_E) # % that is positive
LOS(postE$a_E) # % that is negative
...
p.linkE <- function(postE, R, E){
    
    A = exp(postE$a + postE$a_E*E)

    H = exp(postE$h + postE$h_E*E)
...
R <- seq(from=0, to =25, by =0.01)
C <- sapply(1:length(R), function(j) p.linkE(postE, R = R[j], E = 0))
E <- sapply(1:length(R), function(j) p.linkE(postE, R = R[j], E = 1))
...
```{r}
# plot data
FRE <- ggplot(dfE, aes(x = R, y = median, color = Treatment)) +
...
ids = c(which(names(post)== "pFR[1]"),
which(names(post)== "pFR[951]"))
preditedVals <- post[, ids[1]:ids[2]]

head(preditedVals)

pmed <- apply(preditedVals, 2, median)
pci <- t(apply(preditedVals, 2, HPDI, prob= 0.95))


pddf <- data_frame(Medians = pmed, 
                   lower = pci[,1], 
                   upper = pci[,2],
                   Observed = data$Eaten)


plot(pmed ~ data$Eaten)
abline(0,1)

## plot in ggplot

ggplot(pddf, aes(x = Observed, y = Medians)) + 
  geom_point(alpha = 0.1) + geom_errorbar(aes(ymin = lower, ymax =upper), alpha = 0.1) + 
